...
class FormGenerator {
    ...
    public function __construct($tableName, $model = null) {
        ...
        public function addSelect($name, $label = '', $options = [], $selected = '', $required = false, $disabled = false) {
            ...
            $onchange = ($name == 'state_id') ? "onchange='fetchCountry(this.value)'" : '';


            $this->formHtml .= "<div class='row '>
                                    <div class='col-md-12'>
                                        <label for='$name' class='form-label fw-semibold'>$label</label>
                                        <select class='form-select w-100' id='$name' name='$name' $req $dis $onchange>";
...
$sql = "CALL csms1_search_with_joins(
    ...
    't.account_group_id = t4.customer_account_group_id, t.city_id = t6.city_id, t.state_id = t2.state_id, t2.country_id = c.country_id'
        ...
        // Define columns and tables for the procedure
...
$whereCondition = "ct.city_id = $city_id"; // inject actual value here (not parameterized!)
