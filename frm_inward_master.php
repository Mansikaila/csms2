            ...
            $row = $select->fetch(PDO::FETCH_ASSOC);
             if($row) {
                    ...
                    if($generator_options) {
                        ...
                        $is_active_customer = (isset($_bll->_mdl->is_active_customer) && $_bll->_mdl->is_active_customer == 1);

                        // Check if broker is active
                        $is_active_broker = (isset($_bll->_mdl->is_active_broker) && $_bll->_mdl->is_active_broker == 1);
                                    ...
                                    case "select":
                                        ...
                                        if (!empty($dropdown_table[$i]) && !empty($label_column[$i]) && !empty($value_column[$i])) {
                                            if ($is_active_customer) {
                                                ...
                                                $field_str .= '<select class="' . $cls . '" id="' . $fields_names[$i] . '" name="' . $fields_names[$i] . '" ' . $disabled_str . ' ' . $required_str . ' >';
                                                $field_str .= '<option value="">No active customer</option>'; 
                                                $field_str .= '</select>' . $error_container;
                                            }
                                        }
                                    } else {
                                        $field_str .= '<input type="' . $fields_types[$i] . '" class="' . $cls . '" id="' . $fields_names[$i] . '" name="' . $fields_names[$i] . '" placeholder="' . ucwords(str_replace("_", " ", $fields_names[$i])) . '" value="' . $value . '" ' . $min_str . ' ' . $step_str . ' ' . $chk_str . ' ' . $disabled_str . ' ' . $required_str . ' />' . $error_container;
                                    }
                                    break;
                                    case "hidden":
                                        $lbl_str="";
