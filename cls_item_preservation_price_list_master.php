    ...
    if (!empty($this->_mdl->_array_itemdetail)) {
        ...
        foreach ($this->_mdl->_array_itemdetail as $detailRecord) {
            foreach ($detailRecord as $key => $value) {
                $detailBLL->_mdl->{$key} = $value;
            }

            if (!isset($detailBLL->_mdl->item_preservation_price_list_detail_id)) {
                $detailBLL->_mdl->item_preservation_price_list_detail_id = null;
            }

            $detailBLL->_mdl->item_preservation_price_list_id = $this->_mdl->_item_preservation_price_list_id;
