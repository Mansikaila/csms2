...
$company_state_id = '';
try {
    // Get the state name from company-drashti
          ...
          <div class="form-group row gy-2">
            ...
            <div class="row mb-3 align-items-center">
              <div class="col-12">
               ...
               <div class="row mb-3 align-items-center">
                <?php
        // In the custom layout section for row2_fields
            ...
            if (strpos(strtolower($field), 'sgst') !== false || 
                ...
                <div class="d-flex align-items-center gap-2">
                    <div class="flex-grow-1">
                        <?php echo $custom_fields[$field]['field']; ?>
                    </div>
                    <span class="fw-bold">%</span>
                    <div class="flex-grow-1">
                        <?php 
                        $rate_field = $field.'_rate';
                        if (isset($custom_fields[$rate_field])) {
                            echo $custom_fields[$rate_field]['field'];
                        } else {
...
<script>
    // Pass company state_id as string for JS comparison-drashti
    const companyStateId = '<?php echo htmlspecialchars($company_state_id, ENT_QUOTES); ?>';
</script>   
 <!-- shows, hides, enables, or disables tax fields based on selected 
invoice type(Regular, Tax Invoice, or Bill of Supply).-drashti --> 
<script>
...
// On customer change, fetch customer state_id and update tax_amount radio-drashti
$('#customer').on('change', function() {
    ...
    // Handle HSN code change to fetch tax rates
    $('#hsn_code').on('change', function () {
        const hsnCodeId = this.value;
        if (!hsnCodeId) {
            // Clear tax fields if no HSN code is selected
            $('#sgst').val('');
            $('#cgst').val('');
            $('#igst').val('');
            return;
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.success) {
                    $('#sgst').val(response.sgst_rate || '');
                    $('#cgst').val(response.cgst_rate || '');
                    $('#igst').val(response.igst_rate || '');
                } else {
                    ...
                    Swal.fire({
                        icon: 'error',
                        title: 'Error',
                        text: response.error || 'Failed to fetch tax rates'
                    });
                }
            },
            error: function () {
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: 'Failed to fetch tax rates'
                });
            }
        });
    });

    // Trigger change event on page load if hsn_code has a value
    if ($('#hsn_code').val()) {
...
function handleInvoiceTypeChange() {
    ...
    if (selectedType === "1") { // Regular
        ...
        // Trigger HSN code change to fetch tax rates
        if ($('#hsn_code').val()) {
            $('#hsn_code').trigger('change');
        }
    ...
    // Default to Tax Invoice and focus-drashti
    ...
    <script>
        //Iqty * unit = weight(manual invoice)-drashti
    ...
    if (qtyInput && unitInput && weightInput) {
        qtyInput.addEventListener('input', calculateWeight);
        unitInput.addEventListener('input', calculateWeight);
    }
}); 
    </script>
...
function updateTableRow(index, rowData) {
    ...
    // MANSI- On customer change, fetch lots for that customer
