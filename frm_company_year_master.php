...
$last_year = date('Y'); // Default to current year

try {
    $stmt = $_dbh->query("SELECT start_date FROM tbl_company_year_master ORDER BY start_date DESC LIMIT 1");
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
    if ($row) {
        $last_year = date('Y', strtotime($row['start_date']));
    }
} catch (PDOException $e) {
}

echo "<script>var last_year_start = '$last_year';</script>";
    $transactionmode="";
  ...
  <div class="content-wrapper">
    ...
    <?php
            ...
            $row = $select->fetch(PDO::FETCH_ASSOC);
             ...
             if ($fields_names[$i] === "company_type") {
                                echo '<div class="form-group row gy-2">';
                                echo $lbl_str;
                                echo '<div class="col-4 col-sm-4 col-md-2 col-lg-2 gx-1">';
                                echo $field_str;
                                echo $lbl_err;
                                echo '</div>';
                                echo '</div>';
                            } elseif ($fields_names[$i] === "start_date") {
                                echo '<div class="form-group row gy-2">';
                                echo $lbl_str;
                                echo '<div class="col-6 col-sm-4 col-md-3 col-lg-2 gx-1">'; 
                                echo $field_str;
                                echo $lbl_err;
                                echo '</div>';
                            } elseif ($fields_names[$i] === "end_date") {
                                echo $lbl_str;
                                echo '<div class="col-6 col-sm-4 col-md-3 col-lg-2 gx-1">';
                                echo $field_str;
                                echo $lbl_err;
                                echo '</div>';
                                echo '</div>'; // Close row started by start_date
                            } else {
                                echo $lbl_str;
                                if ($field_str) {
                                    echo '<div class="col-8 col-sm-4 col-md-3 col-lg-2 ' . $cls_err . '">';
                                    echo $field_str;
                                    echo $lbl_err;
                                    echo '</div>';
                                }
...
document.addEventListener("DOMContentLoaded", function () {    
 ...
 document.getElementById("btn_add").addEventListener("click", function (event) {
    ...
    $(document).ready(function () {
        if (!$('#hidden_start_date').length) {
            $('<input>').attr({
                type: 'hidden',
                id: 'hidden_start_date',
                name: 'hidden_start_date'
            }).appendTo('#masterForm');
        }

        if (!$('#hidden_end_date').length) {
            $('<input>').attr({
                type: 'hidden',
                id: 'hidden_end_date',
                name: 'hidden_end_date'
            }).appendTo('#masterForm');
        }

        if (typeof last_year_start === 'undefined') {
            alert("Last year not found!");
            return;
        }

        var existing_years = [2024, 2025]; 

        function updateDates() {
            console.log(last_year_start);
            var companyType = $('#company_type').val(); 
            var startDate = '', endDate = '';

            var baseYear = parseInt(last_year_start);
            var currentYear = new Date().getFullYear();

            if (companyType == '1') { 
                var nextYear = baseYear + 1; 

                while (existing_years.includes(nextYear)) {
                    nextYear++;  
                }
                ...
                var prevYear = baseYear - 1; 

                while (existing_years.includes(prevYear)) {
                    prevYear--;  
                }

                startDate = `${prevYear}-04-01`; 
                endDate = `${prevYear + 1}-03-31`; 
            }
        ...
        $('#company_type').val('1');
        $('#company_type').on('change', updateDates);
        updateDates();
    });
</script>
<?php
