...
if ($fields_types[$i] == "select" && $fields_names[$i] == "company_year_type") {
    ...
    if (isset($_bll->_mdl->{$fields_names[$i]})) {
        $selected_val = $_bll->_mdl->{$fields_names[$i]};
    }
  ...
  if ($fields_names[$i] == "start_date" || $fields_names[$i] == "end_date") {
$disabled_str = 'readonly'; 
    $currentYear = (new DateTime())->format('Y');
    $start_date_val = '';
    $end_date_val = '';

 if (isset($_bll->_mdl->company_year_type)) {
    $company_year_type = strtolower($_bll->_mdl->company_year_type); // Normalize to lowercase

   if ($company_year_type == "forward") {
    $start_date_val = ($currentYear + 1) . '-04-01';
    $end_date_val = ($currentYear + 2) . '-03-31';
} elseif ($company_year_type == "reverse") {
    $start_date_val = ($currentYear - 1) . '-04-01';
    $end_date_val = $currentYear . '-03-31';
}

}
    if ($fields_names[$i] == "start_date") {
    ...
    if (isset($_bll->_mdl->{$fields_names[$i]})) {
        $value = $_bll->_mdl->{$fields_names[$i]};
    }
    $input_type = $fields_types[$i];
}
$field_str .= '<input type="' . $input_type . '" class="' . $cls . '" id="' . $fields_names[$i] . '" name="' . $fields_names[$i] . '" placeholder="' . ucwords(str_replace("_", " ", $fields_names[$i])) . '" value="' . $value . '" ' . $min_str . ' ' . $step_str . ' ' . $chk_str . ' ' . $disabled_str . ' ' . $required_str . ' />' . $error_container;
...
$(document).on('change', '#company_year_type', function() {
    ...
    if (selectedType === 'forward') {
        startDate = new Date(currentYear + 1, 3, 1); // April 1, next year
        endDate = new Date(currentYear + 2, 2, 31);  // March 31, year after
    } else if (selectedType === 'reverse') {
