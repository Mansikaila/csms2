...
$start_date_val = isset($_bll->_mdl->_start_date) ? $_bll->_mdl->_start_date : "";
$end_date_val = isset($_bll->_mdl->_end_date) ? $_bll->_mdl->_end_date : "";
    ...
    if (!empty($dropdown_table[$i]) && !empty($label_column[$i]) && !empty($value_column[$i])) {
        // Define a default value for where_condition_val to avoid undefined variable issues
        $where_condition_val = isset($where_condition[$i]) ? $where_condition[$i] : "1=1";

        $field_str .= getDropdown(
            $dropdown_table[$i],
            $value_column[$i],
            $label_column[$i],
            $where_condition_val, // Ensure this is properly defined
            $fields_names[$i],
            $selected_val,
            $cls,
            $required_str
