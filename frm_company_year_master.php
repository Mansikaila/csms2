                            ...
                            for($i=0;$i<count($fields_names);$i++) {
                                ...
                                switch($fields_types[$i]) {
                                    ...
                                    case "select":
                                        ...
                                        if ($fields_names[$i] == "yearMode") {
                                            $options = [
                                                "current" => "Current Year",
                                                "forward" => "Forward",
                                                "reverse" => "Reverse"
                                            ];
                                            $field_str .= '<select id="'.$fields_names[$i].'" name="'.$fields_names[$i].'" class="'.$cls.'">';
                                            foreach ($options as $key => $val) {
                                                $selected = ($key == $selected_val) ? "selected" : "";
                                                $field_str .= '<option value="'.$key.'" '.$selected.'>'.$val.'</option>';
                                            }
                                            $field_str .= '</select>' . $error_container;

                                        } else if (!empty($dropdown_table[$i]) && !empty($label_column[$i]) && !empty($value_column[$i])) {
                                            ...
                                            $field_str .= '<select id="'.$fields_names[$i].'" name="'.$fields_names[$i].'" class="'.$cls.'">' . $error_container . '</select>';
                                        }

                                    } else {
                                        // Handle non-select input types (text, number, date, etc.)
...
<script>
document.addEventListener("DOMContentLoaded", function () {
    const yearModeSelect = document.getElementById("yearMode");
    ...
    yearModeSelect.addEventListener("change", updateDates);
