...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function calculateFinancialYear(offset = 0) {
        ...
        baseYear += offset;
        const start = new Date(baseYear, 3, 1); 
        const end = new Date(baseYear + 1, 2, 31); 
        return { start, end };
    }
    ...
    function updateDates() {
        let offset = 0;
        if (yearMode.value === "forward") {
            offset = 1; 
        } else if (yearMode.value === "reverse") {
            offset = -1; 
        }
        const { start, end } = calculateFinancialYear(offset);
        startDate.value = formatDate(start);
        endDate.value = formatDate(end);
    }
    updateDates();
    yearMode.addEventListener("change", updateDates);
});
</script>
<?php
