...
document.addEventListener("DOMContentLoaded", function () {
    ...
    gstinInput.addEventListener("input", function () {
        ...
        if (gstinInput.value.length === 15 && gstinRegex.test(gstinInput.value)) {
            gstinInput.classList.remove("is-invalid");
            gstinInput.nextElementSibling.textContent = "";
        } else {
            gstinInput.classList.add("is-invalid");
            gstinInput.nextElementSibling.textContent = "Please enter a valid GSTIN (15 characters).";
            gstinInput.focus(); // Keep focus on the GSTIN input
        }
    });
});
</script>
